...
public final class QueryUtils {
    ...
    private static List<Book> extractFeatureFromJson(String bookJSON) {
        ...
        try {
            ...
            for (int i = 0; i < itemArray.length(); i++) {
                ...
                // for that book.
                JSONObject volumeInfos = currentItem.getJSONObject("volumeInfo");


                // Extract the value for the key called "title"
                String title = volumeInfos.getString("title");

                // Extract the value for the key called "canonicalVolumeLink"
                String url = volumeInfos.getString("canonicalVolumeLink");
                String author = null;

                try {
                    // Extract the value for the key called "author"
                    JSONArray authorArray = volumeInfos.getJSONArray("authors");
                    // For each book in the itemArray, create an {@link Book} object
                    for (int ii = 0; ii < authorArray.length(); ii++) {
                        if (ii == 0) {
                            author = authorArray.getString(ii);
                        } else {
                            author += authorArray.getString(ii);
                        }
                    }
                    ...
                    Log.e("QueryUtils", "Problem parsing the authors JSON results");
                } finally {
                    // Extract the value for the key called "title"
                    author = volumeInfos.getString("publisher");

                }
                // Create a new {@link book} object with the author, title,
     ...
     */
    public static List<Book> fetchUrlData(String requestUrl) {
